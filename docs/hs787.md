...
all of that (`EFBBridgeServer`, `hs787-mfd-bridge.js`, `hs787-efb-bridge.js`, the FS2020
...
def that is not `ExcludeFromMonitorManager` (the silent caches in
`HorizonSim787Definition.CacheOnlyVariables` are stamped excluded in `BuildVariables()` - a row for one would
mute nothing), persisting unchecked keys to `UserSettings.HS787DisabledMonitorVariables`; the def's
...
`HS787_FmsPhase`; `HS787_IRS_Align` has its own return-true branch, so only the wrap mutes it). **If you ever add a monitor manager to another custom-`ProcessSimVarUpdate`
...
`TOTAL WEIGHT` kg) — registered Continuous+IsAnnounced but listed in `HorizonSim787Definition.CacheOnlyVariables`
(SimVarUpdate.cs) - the ONE set that both makes `ProcessSimVarUpdate` return true without speaking AND gets
`ExcludeFromMonitorManager` stamped in `BuildVariables()`; add any new silent cache var there, never as a
hand-written `return true` branch, or it re-earns a dead Ctrl+M checkbox; `MainForm.BuildHs787EicasText` reads them via `GetCachedVariableValue` + the
