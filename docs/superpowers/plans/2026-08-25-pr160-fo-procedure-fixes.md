...
3. **737 landing** — lever DOWN, run the Landing flow. Confirm the lever reaches ARM (the app's own speed-brake monitor announces "Speed brake armed"; L-var `switch_679_73X` reads 100). Repeat by ticking `LDA_SPDBRK` on the Landing group. If either fails, the item must now **revert and report**, not tick silently.
