...
- **⚠️ A one-shot read must NEVER reuse the request id of a standing subscription.** `ExcludeFromBatch` + Continuous + IsAnnounced arms a per-var `PERIOD.SECOND` request keyed on the var's data-def id (`RegisterAllVariables`). Re-issuing a request id with a different period REPLACES that request — that is precisely how `SafelyClearDataDefinition` cancels a recurring one, with `PERIOD.NEVER` — so a `RequestVariable(key, forceUpdate:true)` that reused the id traded the var's live stream for a single sample, with nothing short of a full re-registration to re-arm it. The symptom is silent and delayed: announcements simply stop after the first readout hotkey press, while the readout itself keeps working. **`RequestVariable` now reads such vars on a SEPARATE request id** — `OneShotRequestIdOffset` (200000) + the data-def id, registered in `requestIdToVarKey` against the same var key so the response still routes, and gated on `standingSubscriptionVars` so every other var keeps its existing single-id behaviour. Both ids point at the same data DEFINITION; only the REQUEST id differs, so this costs no extra defs. The offset sits above `INDIVIDUAL_VARIABLE_BASE` (the dispatch routing threshold) and is a request id only, so it cannot collide with `nextTempDefId`'s write-side defs.
