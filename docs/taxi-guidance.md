...
- **Stranded stands are reattached before the component filter is applied (OMDB B 18R, 2026-09-04).** The component filter above is a defence, and at OMDB it misfired: the fs2024 navdata models gate B 18R's stand and its apron connector as a two-node stub whose open end stops **12 m short** of the nearest taxiway-U node instead of meeting it. `Build`'s node merge is 1.5 m, so that stub stayed its own 2-node connected component beside a 3,632-node airport — and because every start-node selector in `LoadRoute` is filtered to the DESTINATION's component, a pilot who asked to taxi to B 18R had **no candidate start node anywhere near the aircraft**. `LoadRoute` answered *"Could not find a nearby taxiway node."* with no route logged at all, and the pilot had to teleport onto their assigned gate. `TaxiGraph.BridgeOrphanParkingIslands` now runs at the end of `Build`, immediately after the first `AssignConnectedComponents` pass: any component that is **not the largest** and **contains a parking lead-in** (an edge whose `PathType` is `P`) is joined to the largest with a **single** bidirectional bridge edge at the closest pair, provided that pair is within `MAX_ORPHAN_PARKING_BRIDGE_M` (50 m — the same distance `TaxiRouter.AStarSearchStrict` already relaxes bridge edges over, describing the same "data seam, not a real gap" phenomenon); components are then renumbered. **Only parking islands are bridged.** A taxiway-only island is the GCLP S5 shape the filter exists to reject, and stitching it would undo that protection; a stand is different in kind, because the sim offers it as a destination so it has to be reachable. **One bridge per island** keeps the island a dead-end spur, so no main-component route can be re-routed through a stand, and the main-side endpoint is never itself a stand nor a node lying only on another stand's lead-in (`LiesOnlyOnParkingLeadIns`) — a bridge landing mid-way down a neighbour's lead-in would steer the pilot along that stand's centreline and across the apron between the two. **"Contains a stand" is keyed on the `P` lead-in edge, never on `TaxiNodeType.Parking`:** `Build`'s parking pass stamps that type on whichever node is nearest a parking spot within 100 m, in any component, so a taxiway-only island running past a stand picks it up by proximity alone, and keying the bridge on it would silently undo the S5 defence. Type `P` rows are exactly the stand lead-ins in both navdata builds (fs2024: all 318,973 end on a `P` endpoint, and no non-`P` row carries one; fs2020 the same). The bridge is unnamed and typed `P` like every other parking connector, and inherits the widest pavement width either end already carries (the steering tone scales its corridor by it). Measured over the whole fs2024 navdata (22,697 airports carrying taxi paths), under the original parking-NODE test, so the lead-in test counts somewhat fewer: 3,655 parking-bearing islands at 2,472 airports, of which 1,205 (33.0 %) sit within 50 m of the main component; median 110.6 m, longest 3,061 m. That long tail is real (remote aprons, farm strips) and is deliberately left disconnected. At OMDB the pass takes the airport from 3 components to 1 and B 18R becomes routable: 1,745 m from K7, clearance honoured, no fallback.
