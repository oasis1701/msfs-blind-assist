...
- **No exit ahead:** *"Taxiway M6, too fast to turn. Slow down."*
  ...
  - its overshoot margin is 0 (`RolloutExitGate.OvershootMarginFeet`): the overshoot handler moves
    ...
    active runway. The handler retargets to a downfield exit (the ORDINARY overshoot search —
    *"Missed taxiway M6. Retargeting …"*; it fires at the node, usually still at 30–60 kt, and
    that search asks for no speed lead, so it may pick an exit that is itself too fast, which then
    makes its own too-fast call at its turn point — see the residuals below), or, with none, starts
    the runway-end countdown WITHOUT *"Missed last exit on runway X."* (the pilot already heard
    ...
    `ComposeRunwayEndStatus`), QUEUED rather than interrupting: the node comes 0.7–3.75 s into
    the 4.39 s *"Taxiway X, too fast to turn. Slow down."*, and an interrupting status cut that
    warning off, possibly before "too fast to turn" was heard. Queued, it follows the warning;
    the interrupting callouts outrank it and carry the same information when they come due first.
...
- **Known residuals.**
  - Once the declined exit is passed, the overshoot handler's cutoff is the aircraft's own position
    ...
    rejected as too close — and, since it fires at the node at 30–60 kt, an exit that is itself too
    fast, which then makes its own too-fast call at its turn point. The search is deliberately left
    as it is.
