...
- **Ground traffic: route, runway, names, queue, faster polling (2026-09-19).** `GroundTrafficMonitor` now reads a `GroundTrafficRouteContext` snapshot from `TaxiGuidanceManager.GetGroundTrafficContext()` (wired as `RouteContextProvider` in MainForm): the route ahead (≤ 2.5 km, from the current segment), the airport's runway centrelines, the runway(s) being held short of (`_heldRunwayLabel`, set by all three hold entries and cleared by `SetState` on leaving `HoldShort`) or lined up on, and whether the route is a departure. Pure geometry and phrasing live in `Services/GroundTrafficLogic.cs`, pinned by `GroundTrafficLogicTests`. What it adds:
  - **Runway watch** while holding short of / lining up on a runway: aircraft on its pavement (`RunwayShape`, any speed) and airborne aircraft on final to either end (≤ 6 nm, inside a widening approach cone, heading within 30° of the landing direction, below 800 ft + 500 ft/nm above field — field elevation is the own aircraft's altitude). One status sentence on arrival at the hold ("Runway 27: no traffic seen on the runway or on final." — deliberately never "clear", which is ATC's word), then changes only: a new aircraft on the runway, a new aircraft on final, the first "short final" (≤ 2 nm), and the runway emptying. While LINING UP a new runway occupant or short final INTERRUPTS; at the hold they queue. The watch is decided BEFORE the sweep is requested (the intake only keeps airborne/far runway aircraft while a watch is active), and the first status waits for a sweep requested after the watch started, or it would claim "no traffic on final" from a sweep that filtered finals out.
  - **Route-aware alerts**: traffic within 30 m of the route ahead (15–600 m along it) gets one "Delta A320 on your route, taxiway B, 800 feet ahead, coming toward you." per episode (re-armed once it is 50 m off the route). With a route to judge by, "Slow down"/"Stop" need a real threat — near the route ahead, a predicted closest approach < 60 m within 30 s, or inside the fixed 250 ft — otherwise they drop to an Awareness ping (which can still escalate later). No route context → the old distance-only behaviour.
  - **Converging**: closest point of approach from both headings and speeds; moving traffic predicted within 45 m in 5–40 s gets "… converging from the left, about 20 seconds." once per episode.
  - **Names**: every callout names the aircraft ATC-style via `GroundTrafficLogic.SpokenName` (airline + spoken type, "DAL 123, 737", type alone, or "traffic"), plus its movement.
  - **Departure queue**: on a departure route, while nearly stopped, aircraft stopped on the route ahead (≤ 1.5 km) are counted — "Number 3 in the departure queue.", confirmed over two sweeps, spoken on change, "First in the departure queue." only after having been further back. Alt+G appends the queue position and the runway status.
  - **A Progressive Taxi leg that ends at a runway hold is a departure like any other** (`TaxiGuidanceManager.ProgressiveHoldRunway`). `WatchedRunways` was populated only from `_heldRunwayLabel`, which only `HoldShort` sets, and `IsDepartureRoute` was `_isRunwayLineup` alone — so the single most common VATSIM shape ("BAW25, taxi via Alpha, AY1, **hold A1**", then "line up" minutes later) got neither the runway watch nor the queue count, in the window where both matter most. A leg whose terminator is **hold short of runway X** takes X outright; one whose terminator is a **named holding point** takes the runway from the resolved node's own `HoldShortName` via `ExtractRunwayDesignators` — Build already worked that out for every hold node, and reading it back is the only thing that works: a second `MatchHoldShortRunwayName` call at the 150 m naming tolerance resolves EGLL A1 (139 m from the 27R centreline) and A4 (138 m) but NOT A3 (158 m) or A2 (186 m), because CAT III holds sit a long way back — Build's own nearest-runway-start fallback covers exactly those, so its label names 27R for all four ("A2, Runway 27R", the shape `RouteRunwayCrossingsTests` pins). The geometry call remains only for a point resolved onto a node with no label at all (an edge projection). The **queue counts for the whole leg** (that is when the queue forms); the **watch arms at the hold only**, matching the HoldShort case rather than reporting a runway through a two-kilometre taxi. Both are computed on demand from the terminator + route + graph, so there is no fourth piece of state for `LoadRoute`'s `Fail` snapshot to leave stale.
  - **Parallel holding queues fall out of the route test, not a special case.** EGLL 27R is fed by A1/A2/A3 as three separate full-length holding points and Tower re-sequences between them, so the pilot is not in one nose-to-tail line. The queue counts only aircraft within `ON_ROUTE_LATERAL_M` (30 m) of *your* route ahead, so a lane you are not routed down is already excluded, and an aircraft ahead that peels off to another holding point simply stops being counted. Nothing here follows another aircraft — EGLL's own vMATS forbids "follow" inside the runway holding area for exactly that reason.
  - **Queue moving, and the "move up" nudge.** The closest aircraft directly ahead (±30°, ≤ 600 ft) that pulls away while the pilot is nearly stopped gets one *"British Airways A320 ahead is moving."* `GroundTrafficLogic.QueueDeparted` has TWO triggers and both are load-bearing: the SPEED EDGE (stopped sample → rolling sample, gate lowered 3.0 → 2.0 kt) and the GAP (the distance has grown ≥ 60 ft over the closest we were while it sat stopped ahead of us, `StoppedGapFt`). A real holding-point queue CREEPS — an aircraft that eases to 2 kt and holds there shows at most one edge, and once its previous sample is above the stopped gate the edge can never fire again, so the edge alone left the pilot sitting behind an opening gap in silence. The gap trigger requires the TARGET to be moving (≥ 1 kt) or our own pushback would open the gap and report it as the queue moving. Afterwards, if the pilot stays put (own GS ≤ 5 kt), *"Move up. 400 feet to the traffic ahead."* repeats every 20 s, up to 3 times, and becomes *"Move up. The traffic ahead has taxied on."* once that aircraft is out of the sweep (gone is a BIGGER gap, not a reason to go quiet). It disarms the moment the pilot moves, and is re-armed by the next aircraft that pulls away. **The nudge is SILENT whenever guidance is deliberately holding the aircraft** (`GroundTrafficRouteContext.IsHolding`: short of a runway, at a Progressive Taxi terminator, or lined up; gating on `WatchedRunways` instead would have missed the progressive holds, which populate none) — because there "Move up" is an instruction ATC has not given, and an aircraft ahead crossing or departing is exactly when a blind pilot must NOT roll. The one-shot "is moving" call is deliberately NOT gated that way: it is information, not an instruction.
  - **One interrupting callout per evaluation**, chosen by priority (Stop > Slow down > converging > on route > awareness > queue moving): `AnnounceImmediate` cuts off whatever is speaking, so a second one only erased the first. Candidates not spoken are not marked and re-fire next sweep. Informational lines (runway status, queue) use the non-interrupting `Announce`.
  - **The 15 s re-announce window now suppresses only Awareness** — it used to swallow a "Stop" arriving within 15 s of that aircraft's "Traffic" ping.
  - **Polling**: 1 s tick; the traffic sweep runs every tick while any ground aircraft is within 1500 ft or a runway is watched, every third tick otherwise; alerts evaluate on the sweep's completion. The sweep radius is unchanged (shared with TCAS — see `RequestAiTrafficData`). Everything spoken is logged to `ground_traffic.log`.
