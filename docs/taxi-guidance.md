...
**The start hold.** When the only stop is the route's start node, the route records `TaxiRoute.StartHoldRunway`. Whether it may is decided by the pass itself, from the aircraft's own state — there is no `allowStartHold` flag any more (PR #238 deferred finding §2): **not while the aircraft stands on any runway's pavement** (`RunwayUnder`), **not once it has rolled more than 10 m past the start node**, and **not while it is rolling** — above `RolloutExitGate.NoExitStoppedGroundSpeedKts` (3 kt) the aircraft is committed to where it is going and a stop at its own position is not an instruction it can take. That last term is what keeps a recalculation and a landing-rollout route from starting held: both are built from a moving aircraft (off-route detection does not even fire below `OFF_ROUTE_MIN_GS_KTS`), so neither needs a flag to say so. `LoadRoute`'s `landingRolloutRoute` parameter now only LABELS the crossings log line `phase=touchdown`.
