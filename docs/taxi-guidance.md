...
- **The landing-exit plan's runway is checked against the runway actually landed on (OMDB, 2026-09-12 / 2026-09-06).** `LandingExitPlanner` deliberately does not depend on which runway the SIM reports, and that is still true — but it used to hand the PLANNED runway to `BeginLandingRollout` as a measurement FRAME without checking it described the aircraft. The planner dialog's runway combo defaults to `SelectedIndex = 0`, which at OMDB is 12L; a pilot who planned exit M13 against that default and then landed on **30L** gave the rollout a frame rotated 179 degrees and offset onto the other runway. On the FIRST update `hdgDelta=179.2deg`, `signedAlongPast=+4570ft` and `lateral=1263ft` — so `pastExit` AND `exitedLaterally` both read true and `UpdateLandingRollout` handed off to `Taxiing` on the frame the rollout began. No touchdown callout, no 1500/500 ft approach calls, no turn cue: the pilot heard taxi guidance naming taxiways instead, which is exactly how it was reported. `Navigation/LandingRunwayMatch` (pure, xUnit-pinned) now classifies the touchdown against the planned runway's own geometry — on its pavement (half-width + 15 m laterally, 50 m of along-track slop at each end) and rolling its way (within 90 degrees): **`Matches`** is today's behaviour, unchanged; **`ReciprocalEnd`** (right pavement, wrong end) swaps `_runway` to the landed end and RE-MEASURES the pilot's exit from it — `DistanceFromThresholdFeet` and `ExitAngleDegrees` are both direction-dependent, and the angle is not cosmetic (it derives `ExitType`, opens the exit-turn gate at `ExitAngleDegrees * 0.7`, and gates `TryEarlyExitHandoff` below 50 degrees, so a 30-degree RET read from the far end is really 150); **`DifferentRunway`** cancels the plan and enters `BeginRunwayEndCountdownRollout` on the runway the aircraft IS on, so a blind pilot at 160 kt still gets “Runway end in 1,500 feet / 500 feet, slow down / 100 feet, stop” and the backtracking handoff; **`Unknown`** (no runway identifiable, or a correction that cannot be resolved to a runway row the caller can act on) keeps today's behaviour rather than inventing one. **Both corrections are ONE utterance.** The touchdown callout goes out through `AnnounceInstruction`, i.e. `AnnounceImmediate`, which DISCARDS the queue — so the reciprocal note rides IN it via `BeginLandingRollout`'s new `touchdownPrefix` parameter rather than being spoken beside it, and the cancelled-plan line is itself an `AnnounceImmediate` that REPLACES the touchdown callout on that path. This is the same two-announcements-stomp-each-other trap the import summary, the length advisory and the reach warning all had to be folded out of. **`LandingRollout` now starts its own position stream.** `OnTaxiGuidanceStateChanged` started `StartTaxiGuidanceMonitoring` only on the `Taxiing` transition, on the reasoning that a rollout always arrives via a route load — true for `BeginLandingRollout`, false for `BeginLandingRolloutNoGraph` (reached after `LoadRoute` FAILED) and for the new countdown entry, neither of which passes through `Taxiing`. Without it those paths sit in `LandingRollout` with `UpdateLandingRollout` never called: an announcement, then silence, on an active runway. Re-issuing request 507 at `SIM_FRAME` is idempotent, so the common path is unaffected.
