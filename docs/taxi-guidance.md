...
2. Planner calls `TaxiGraph.GetLandingExits(runway)` → a distance-sorted list of exit taxiways with classification. The angle is the exit's whole BRANCH — the sharpest turn from the landing heading needed to get clear of the runway, capped at 90° — not the first segment that carries its name (see **Exits measured by branch** below):
...
```
window = half-width / tan 15°  +  |exit node's lateral offset| / tan(exit angle),  capped at 1,000 ft
```
...
- **The fixed 1,000 ft was this sum's worst case**: a 200 ft runway, a marker 45.5 m off (half-width
  ...
  bound in `IsVacateAwayFromPlannedExit`.
...
- The rollout keeps it in `_rolloutExitTurnWindowFeet`, recomputed by `UpdateRolloutExitTurnWindow`
  after EVERY assignment of the targeted exit — the two rollout entries that target an exit, every
  retarget, the early-vacate swap — and reset to the fixed 1,000 ft where the exit is cleared
  (`EnterRunwayEndCountdown`, `StopGuidance`). Each recompute for a targeted exit is logged (see
...
- **No exit ahead:** *"Taxiway M6, too fast to turn. Slow down."*
  ...
  - its overshoot margin is 0 (`RolloutExitGate.OvershootMarginFeet`): the overshoot handler moves
    ...
    `ComposeRunwayEndStatus`). That first STATUS — the stopped notice, a milestone or the owed
    one — waits until "too fast to turn … Slow down." has been spoken:
    `RolloutExitGate.CountdownStatusMaySpeak`, 5.3 s after that sentence started
    (`TooFastNoExitSpeechHoldSeconds`; *"Taxiway M9, too fast to turn. Slow down."* measures 4.39 s at
    System.Speech Rate 0, plus about a fifth), because a pilot averaging over ~20 kt across the last
    150 ft reaches the node before it ends. The hold only DEFERS — a held stopped notice is re-decided
    on the first frame past it — and it applies to this path alone; a backtrack or "Runway vacated"
    is not held.
...
- **Known residuals.**
  - Once the declined exit is passed, the overshoot handler's cutoff is the aircraft's own position
    (or 100 ft past the exit) with no lead ahead of it, so it can offer an exit the too-fast scan
...
|---|---|---|
| `Missed` | an overshoot (the rollout's own detector, or the post-handoff monitor in `Taxiing`), and any fall-forward after a route failure except a too-fast one | *"Missed taxiway M6.[ Straighten.] Retargeting taxiway M7, 650 feet ahead.[ Slow down.]"* |
...
| `Earlier` | the undershoot retarget to an earlier exit | *"Taking earlier exit, taxiway A5, 900 feet ahead.[ Slow down.]"* — never "Straighten." |
...
3. **The measured path** runs from the junction to the first node beyond half-width +
   ...
   (`TurnToClearDeg`), capped at 90° (`RolloutExitGate.MaxUsableExitTurnDeg`).
...
taxiway: unnamed edges, or edges carrying the exit's name (`OrdinalIgnoreCase`; an unnamed exit
follows unnamed edges only). A branch that cannot reach or clear the runway that way is unmeasured.
