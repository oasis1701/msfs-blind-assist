...
- **Warning is spoken AFTER `StartGuidance`, by the form.** Announcing it inside
  ...
  *after* `StartGuidance`. The spoken summary is **skipped** when a reach warning
  ...
  warning is pending — holds the advance turn/taxiway-change notice, the "… ahead."
  callout and the curve cue, which wait and speak normally once the window
  closes; the taxiway-crossing callout is handled and skipped while it's open,
...
- **Stranded stand stubs are reattached; unreachable destinations are refused (OMDB B 18R, issue #228; narrowed in review 2026-09-14).** The component filter above misfired at OMDB. Gate B 18R's stand and connector are one `P` lead-in row whose open end stops 12 m short of taxiway U, so the stub stayed its own component and `LoadRoute` found no start node (*"Could not find a nearby taxiway node."*).
  ...
  **Why it was narrowed.** The first version bridged any component with one lead-in. Measured with the real `Build` over all fs2024 airports, that produced 1,144 bridges at 852 airports, 554 of them touching runway pavement. Route replays showed:
  ...
  Narrowed, and re-run over every airport that sweep found with a stand island, it gives 234 bridges at 112 airports. None is on or across a runway, or lands on a hold-short node, a stand or a lead-in chain, and no landing exit or downfield exit changes. The one backtrack-entrance difference is at WABI, where a bridged stand stub is the graph node nearest a runway end, so the entrance becomes reachable from that end. OMDB C 51L stays unbridged: the only node within 50 m of its stub is a bend on the neighbouring stand's taxiway Z lead-in. 13 network ends lie within 150 m of a runway centreline, which is the residual exposure of the post-landing vacate walk.
  ...
  **Reachability.** `LoadRoute` and `TryRecalculateRoute` ask `RouteReachability` where the aircraft is before adopting a route.
  ...
  - **No start node in range** for a destination off the network: refused by name (*"No taxi route to X. It isn't connected to the taxiway network you're on."*) instead of the old *"Could not find a nearby taxiway node."*
  - **Delivery.** Turn, taxiway-change, destination-ahead and curve callouts wait until the start grace window (`START_WARNING_CHATTER_GRACE_SEC`, 12.5 s, sized from a measured 10.4 s warning plus turn cue) closes. The window opens for the runway reach warning and for this warning, so a callout at a standstill can no longer cut either off. A refused load puts back the destination, lineup and graph state it had overwritten, so a refusal while taxiing leaves the route in progress untouched.

  At LFBP, Parking 40 from taxiway N3 used to start 265 m away across runway 13/31; it is now refused. Across fs2024, 13,234 stands at 2,430 airports sit on a piece of taxi network the main one does not reach (the bridges bring 239 more onto it), so a route to them from the main network starts with the unmapped-leg warning, or is refused when that leg touches a runway.
