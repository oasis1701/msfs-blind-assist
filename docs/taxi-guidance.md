...
- For each route segment whose **edge** (FromNode→ToNode) crosses a runway centerline — `RouteRunwayCrossings.EdgeCrossesRunway` (a proper segment-vs-centerline intersection, against the runway's PAVEMENT endpoints) via `WhichRunwayCrossedByEdge` — tag the hold segment resolved by `RouteRunwayCrossings.ResolveCrossingHoldSegment` (see below) as `IsHoldShortPoint = true` with `HoldShortRunway = "runway X"`.
...
**Which LINE the crossing is tested against: the PAVEMENT, never the start rows** (`RouteRunwayCrossings.EdgeCrossesRunway` — the ONE owner, called by `WhichRunwayCrossedByEdge`, `ApplyUserRunwayHoldShorts` and `RolloutRunwayReCrossing.CrossesRunway`). All three used to intersect against `RunwayCenterline.Lat1..Lon2`, which comes from the navdata `start` table; `SnapStartToRunwayCenterline` repairs those rows only LATERALLY, so at a displaced threshold the row sits far inside the pavement and **the band between the two is real runway no crossing test could see**. Measured at OMDB: 12R/30L's start-row line is 2,779 m against 3,724 m of pavement — **761 m invisible at the 12R end, 184 m at the 30L end**, with taxiways **K5, K6, K7, M8 and K16** crossing inside it; 12L/30R loses 496 m and 103 m, taking **N1, N1A, N1C, M1B and N9**. Three safety behaviours failed together there: the auto pass logged `crosses=(none)` and placed **no hold-short at all** for a route crossing in that band (the pass CLAUDE.md says must never be disabled, disabled by geometry rather than by code); an explicit *“hold short of runway 12R at K6”* pick was refused with *“route does not cross it after taxiway K6”*; and the landing-exit handoff guard could not see a re-crossing there either. **Strictly wider, never narrower** — the pavement line CONTAINS the start-row line, so every crossing found before is still found; where `Build` had no runway table the `Pavement*` fields are a copy of the start rows, and the helper additionally falls back to them if the pavement pair is not a usable line (a `RunwayCenterline` built by hand leaves them at (0,0), and a degenerate line at null island crosses nothing — silence in the UNSAFE direction). Measured effect at OMDB: the validator's refusals of taxiways the form OFFERS drop from 9 to 5 on 12R and 7 to 3 on 30L. **This is not an OMDB quirk.** Swept over the whole fs2024 navdata (22,697 airports carrying taxi paths): **14,288 runway crossings, on 8,062 runways at 6,831 airports**, cross the pavement while the start-row line cannot see them — 4,063 of those runways have a NAMED taxiway among them, so they are pickable in the form and were refusable. The worst-end blind band runs a median of 32 m and a p90 of 235 m, with a long tail: OIII 11R/29L hides **3,969 m**, URWW 05/23 2,854 m, KLRF 07/25 1,541 m. **Deliberately NOT extended to hold-short NAMING** (`MatchHoldShortRunwayName`) or `DescribeLocation`, which answer different questions with their own tuned tolerances.

**Known residual, measured and left alone:** `TaxiAssistForm.GetTaxiwaysCrossingRunway` (the per-row picker's LIST) still uses its own geometry — a cross-track SIGN CHANGE against `RunwayFrame.For(runway, …)` within `along ∈ [-50, length+50]` — so it stays slightly more permissive than the validator and can still offer a taxiway the validator refuses. Every remaining OMDB disagreement is the PICKER being too permissive, not the validator: K3 (–27 m) and K4 (−1 m) cross BEFORE the 12R threshold, K17/K18/N10 cross BEYOND the far one, and M18 has an endpoint 0.04 m off the centreline — it JOINS the runway rather than spanning it, and a sign test cannot tell those apart. Closing the gap is not purely mechanical: a taxiway that ENDS at a runway is a legitimate place to be told to hold short even though it is not a crossing, so making the picker call `EdgeCrossesRunway` would drop M18 from the list. That is a semantics decision (entrance vs crossing) and wants its own change.
