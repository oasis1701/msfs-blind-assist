...
already decided. `Describe` owns the wording, including the rule that an
...
**A route that ends on the runway's own centreline BEHIND its threshold reaches it (OMDB 12R via K1, 2026-09-05).** SayIntentions cleared *"taxi to holding point runway 12R via U, W, Z, Z9, K, K1"*; the router honoured it in full (73 nodes, no fallback) and ended on K1, and the reach gate answered *"this route stops short of Runway 12R … the last taxiway you entered does not connect to it … reprogram"*. Measured against the reporter's own fs2024.sqlite: the route end (node 571) is **20.1 m** off 12R's centreline and **699 m behind its threshold**, and the graph walk to the modelled pavement is **1,018 m** — over `RUNWAY_REACH_MAX_WALK_M`. But X-Plane's apt.dat gives 12R a **713 m displaced threshold** and puts its physical pavement start **31 m** from that node: MSFS models a runway from the DISPLACED threshold onward, so the whole full-length departure end of 12R is pavement the navdata does not call runway at all. The clearance was right, “reprogram” was unactionable (there is no other taxiway to enter) and “does not connect” was false. `TaxiGraph.MetersBehindRunwayThreshold` now measures how far a node sits behind the departure threshold while still inside the runway's own lateral corridor (half-width + 5 m, the same corridor `GraphWalkToRunwayPavement` uses), and `RunwayReachGate` answers `ReachesBehindThreshold` for it — a REACHING verdict with an advisory instead of a warning, placed ABOVE the walk probe because it is cheaper and because the walk gives the wrong answer here by construction. **The lateral corridor is the safety property, not a detail.** The fault the gate exists to catch is a clearance that ended on a taxiway PARALLELING the runway (PHNL 04L, 655 m of taxiing with no connector), and a parallel taxiway is laterally OFFSET — so it can never be inside the corridor, never reports a distance here, and nothing that warns today stops warning. There is deliberately **no distance cap**: the corridor already bounds the answer to the runway's own axis, and a cap is the kind of tuned constant this codebase has learned not to guess at. Blast radius, measured over the whole fs2024 navdata (22,697 airports carrying taxi paths): **7,716 runway ends at 6,048 airports** have any behind-threshold corridor node at all, and they are overwhelmingly the blast pad right behind the paint — **median 40 m, p90 233 m**, with 90.7 % inside 250 m. OMDB 12R's 699 m sits in the top 2.5 %. The verdict additionally requires a ROUTE to END on such a node, so the real exposure is far smaller than that bound. The lineup is correct from there without any further change — `UpdateLineup` is intercept-angle work against the centreline, which is the same line 699 m back — and `_routeReachesRunway` now accepts the new verdict, which only disarms the during-lineup bailout, itself gated on a cross-track (≈122 ft) the 20.1 m end never reaches. **Rejected, with the measurements, so it is not re-attempted:** a bare “corridor + distance bound” is not enough on its own for a more ambitious version of this (teaching the graph that the pre-threshold pavement IS runway) — OMDB 12R's corridor also contains taxiway L and three GATES 2,182–2,328 m back, and 30R's contains 800 m of taxiway S lying within 8 m of the extended centreline; a gap-limited run does not separate them either (30R's gap rule yields a 593 m “extension” made of the crossing taxiways M14A, N10, M15, N11, N12, M15A, R1 and R). Graph contiguity cannot be used as the discriminator at all: **the taxi graph contains no runway-length pavement**, only the taxiways that cross it (no `taxi_path` row has `type='R'`), so a corridor-only walk from OMDB 12R's 251 on-runway nodes reaches ZERO behind-threshold nodes — not even the one 1 m back. That is why this change is scoped to the REACH VERDICT and does not move the lineup point or the destination node.
