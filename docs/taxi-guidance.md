...
`TaxiGuidanceManager` stores the preferred runway hold-short taxiway from the explicit route load (`_preferredRunwayHoldShortTaxiway`). Automatic off-route recalculation must reuse that stored value instead of deriving a new one from the remaining route suffix. Example: if the pilot loads `F2, F3, F OUTER, N, N2` to a runway, recalculation near `N` must still stop at the `N2` holding point even if the rebuilt route shape changes. A fresh explicit route load with a different last taxiway, such as `N3`, updates the stored preference; non-runway route loads clear it.
