...
| SC-6 | Dead API cluster: `RequestLVarValue` (no-op body), `RequestSpecificLVar` (zero callers, hash-collision request-ID scheme), `GetAircraftPosition` duplicate (`Monitoring.cs:385-402`), `pendingRequests` dictionary, and the 400-499 dispatch branch (`Dispatch.cs:53-108`). AMENDED at planning: the branch is provably unreachable (its only producer, `RequestSpecificLVar`, has zero callers, and the branch acts only on `pendingRequests` entries — the implementation plan requires verifying this shape before deleting), so the whole cluster is SAFE in PR-2. Its `ecamMasterWarning/-Caution/-Stall` side-writes therefore had no live writer even before removal — record as a latent pre-existing finding for the PR-9 in-sim ECAM check; do not fix in PR-2. | SAFE (with reachability proof) |
...
- **PR-2 `chore/dead-code-removal`** — SC-4 (EFBBridgeServer + HS787SimBriefForm + comment), SC-5 (GenericBatch struct), SC-6 (full cluster incl. the 400-499 branch, per the reachability-proof amendment), SC-8, ND-3 (AirportDatabase), JS-6 (bridge JS + dead patcher halves), JS-8, FM-7, AC-16 dead items, stale comments (SV-7's GsxGateSelector line), DOC-11 (align the three EFBBridgeServer doc mentions to "deleted"). Build must stay green; grep-verified zero references per deletion.
...
- **PR-9 `fix/behavioral-cleanup`** — AC-2 (777 IsReady), AC-4 (autobrake announce), AC-5, AC-6 (HS787 state-aware combos), AC-7 (RMP uniquifier), AC-11 wording harmonization + TryParseSpeedInput port + LAND-speech drift, AC-15 Thread.Sleep removal, FM-5 (validation announce normalization), SV-4 (VG deltaTime), SC-12 (hotkey readout defs), and the ECAM master-warning latent-writer investigation surfaced by SC-6's removal (in-sim check whether the A32NX ECAM window ever received those flags). May split further at implementation time if the test session gets too big.
