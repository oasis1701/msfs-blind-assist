...
**Transport (2026-09 — Coherent primary, SimBridge fallback):** `Services/FlyByWireMCDUService` is a facade over TWO transports the window never sees. PRIMARY is `SimConnect/CoherentA32nxMcduClient`, a persistent Coherent debugger socket on the MCDU view (`A32NX_MCDU`; the Headwind A330's `A339X_MCDU` — `FlyByWireA320Definition.FlightInfoMcduView`), which installs `Resources/coherent-a32nx-mcdu-agent.js` once and polls its `read()` every 250 ms while the window is visible (`SetActive`, wired to `VisibleChanged` in `ShowFlyByWireMCDUDialog`; the socket and agent stay WARM while the window is closed). `read()` rebuilds `A320_Neo_CDU_MainDisplay.sendUpdate()`'s relay payload from the SAME fields on `document.querySelector("a32nx-mcdu").fsInstrument.legacyFms` (`_labels`/`_lines`/`_title`/`_pageCurrent`/`_pageCount`/`_arrows`/`scratchpadDisplay`/`annunciators`, power-gated on `A32NX_ELEC_AC_ESS_SHED_BUS_IS_POWERED` / `AC_2`), so ONE parser, `Services/FbwMcduUpdate`, decodes both transports and `tools/a32nx-mcdu-agent-test` (node, CI) replays `sendUpdate()`'s construction against a stub to pin that parity. Keys go through `press(key)`, which dispatches `A320_Neo_CDU_1_BTN_<KEY>` on the instrument's OWN H-event publisher (`fsInstrument.hEventPublisher.dispatchHEvent`, exactly how the sim delivers the relay's `SimVar.SetSimVarValue("H:…")`), falling back to `bus.pub("hEvent", …)` and then `legacyFms.onEvent` — no MobiFlight dependency. A press that PROVABLY never reached the instrument (no socket, no agent, no instrument, no dispatch path — `CoherentA32nxMcduClient.IsUndeliveredKey`) is resent over the relay when SimBridge is up; a timeout or a dispatch that threw is ambiguous and is never resent, since a second press on an MCDU key is its own error. Every non-delivery is logged (`A32NX MCDU key <KEY>: …`). When the page re-evaluates and loses the agent, `read()`/`ping()` answer `no-agent` rather than nothing, so the agent is re-installed on the SAME socket; only evals answered by nothing count toward the dead-socket teardown. FALLBACK is `Services/FlyByWireSimBridgeMcduClient`, the SimBridge relay websocket `ws://localhost:8380/interfaces/v1/mcdu` this window used exclusively before (sim → client `update:{left:{...},right:{...}}`, each side `lines` 12×3 `[left,right,center]` even=label/odd=value, `scratchpad`, `title`, `page`, `arrows` `[up,down,left,right]`, `annunciators`; client → sim `event:left:<KEY>` / `requestUpdate`; the gateway echoes every message to every client). It feeds the window only while the Coherent socket is down, and it is still the ONLY source of MCDU printouts (`print:` exists nowhere but on the relay). `Services/FbwMcduTransportArbiter` (pure, `FbwMcduTransportArbiterTests`) decides which is live: Coherent whenever connected, a frame from the non-live transport is remembered but never published (two sockets narrating one screen a poll apart reads as flicker), a switch re-publishes the newcomer's last frame — never one from before that transport's own disconnect, which is discarded (the screen may have moved on while it was down, and both transports push a fresh frame on reconnect) — and the window's "MCDU connected" is "either transport up", reported on change only. WHY: SimBridge is a separate process the sim never launches, it crashes or hangs on long sessions, and a hung SimBridge still reads as connected to the aircraft (`McduServerClient` reconnects only on socket close), so the MCDU was simply gone until it was restarted. ⚠️ Coherent GT allows ONE inspector socket per view: once the MCDU window has been opened the service holds the `A32NX_MCDU` socket for the aircraft session, so the D / Shift+D flight-info script rides `FlyByWireMCDUService.EvalOnMcduViewAsync` (`MainForm.AnnounceA32NXFlightInfo` checks `HoldsMcduView`) and the one-shot `CoherentEvalClient` eval is used only before the window has ever been opened — a second socket on that view is refused. The old "no persistent Coherent socket on the A32NX" policy was retired by the maintainers on 2026-09-25.

**Key components:**
- **`Services/FlyByWireMCDUService.cs`** — the facade (see Transport): owns `SimConnect/CoherentA32nxMcduClient` + `Services/FlyByWireSimBridgeMcduClient`, drives `FbwMcduTransportArbiter` on the UI context (both clients post there), forwards `DisplayUpdated`/`ConnectionStatusChanged`/`PrintReceived`, routes `SendButtonPress` to the live transport, and exposes `HoldsMcduView`/`EvalOnMcduViewAsync` for the shared-socket rule. The class remarks document the no-separation limitation for BOTH transports.
