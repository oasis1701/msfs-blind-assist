...
| `FlareAltitudeBiasFt` | 12 *(est.)* | 20 *(measured)* | Subtracted from measured altitude before the flare/touchdown thresholds so they fire at true main-gear height. See below. |
...
| `flareAltitudeBiasFt` `[profile]` | 12 *(est.)* | Subtracted from measured altitude before flare/touchdown checks → true gear height. PMDG 777 = 20 (measured). |
| `FLARE_ALTITUDE_FT` | 30.0 | Flare phase entry — true main-gear height (`wheelHeightAGL`) |
