...
namespace MSFSBlindAssistUpdater
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                string logPath = Path.Combine(Path.GetTempPath(), "MSFSBlindAssist_Updater_Args.log");
            ...
            if (args.Length != 3)
            {
                ...
                MessageBox.Show(
                    "Invalid arguments.\n\n" +
                    "Usage: MSFSBlindAssistUpdater.exe <zipPath> <appDirectory> <appExecutablePath>\n\n" +
                    diagnosticInfo + "\n" +
                    $"A log file has been saved to:\n{Path.Combine(Path.GetTempPath(), "MSFSBlindAssist_Updater_Args.log")}",
                    "MSFS Blind Assist Updater - Error",
