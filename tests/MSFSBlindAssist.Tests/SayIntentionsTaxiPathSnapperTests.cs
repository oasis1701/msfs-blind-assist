...
public class SayIntentionsTaxiPathSnapperTests
{
    ...
    // --- A cleared taxiway is not an excursion ------------------------------------------
    //
    // The excursion filter is geometric and cannot tell a junction clip from a cleared leg
    // that happens to be brief. Live CYVR, 2026-08-19: the track read
    // [6, JB, 6, J, H, D, D5, D, D9] against a clearance of [JB, J, H, D, D9]. JB is
    // cleared, is sandwiched between two 6 runs, and appears nowhere else — so removing it
    // breaks the clearance walk, the clearance wins, and the pilot is told the ground track
    // differs from a track that agreed with it.

    /// <summary>The CYVR shape: X, Y, X, Z — Y sandwiched and occurring exactly once.</summary>
    private static GeoPoint[] CyvrShape() => new[]
    {
        OnMain(10), OnMain(20), OnMain(30),
        OnSpur(1), OnSpur(2),
        OnMain(80), OnMain(90),
    ...
    [Fact]
    public void ASandwichedRunTheClearanceNamedIsKept()
    {
        var result = SayIntentionsTaxiPathSnapper.Snap(
            CyvrShape(), JunctionEdges, new[] { "Y", "Z" });

        Assert.Equal(new[] { "X", "Y", "X", "Z" }, result.Taxiways);
        Assert.Equal(0, result.ExcursionRunCount);
    }

    [Fact]
    public void TheSameRunIsRemovedWhenNoClearanceIsSupplied()
    {
        // The pin for the test above: without the cleared set, this shape is an ordinary
        // excursion and drops. If this ever agrees with the test above, the parameter has
        // stopped doing anything.
        var result = SayIntentionsTaxiPathSnapper.Snap(CyvrShape(), JunctionEdges);

        Assert.Equal(new[] { "X", "Z" }, result.Taxiways);
        Assert.Equal(1, result.ExcursionRunCount);
    }

    [Fact]
    public void ASandwichedRunIsStillRemovedWhenTheClearedNameSurvivesElsewhere()
    {
        // "via A, B" where A and B cross: the track travels A, clips B at the junction,
        // returns to A, then genuinely taxis B. Protecting every cleared name would keep
        // the spurious out-and-back; the occurs-exactly-once test is what prevents that,
        // because the second B carries the evidence on its own.
        var path = new[]
        {
            OnMain(10), OnMain(20), OnMain(30),
            OnSpur(1), OnSpur(2),
            OnMain(80), OnMain(90),
            OnSpur(6), OnSpur(7), OnSpur(8),
        };

        var result = SayIntentionsTaxiPathSnapper.Snap(path, JunctionEdges, new[] { "X", "Y" });
    ...
    [Fact]
    public void ASandwichedRunTheClearanceDidNotNameIsStillRemoved()
    {
        // The clearance names the anchor but not the spur — the ordinary case, and the one
        // that must not regress: 17 of the 18 sandwiches measured in the live log are this.
        var result = SayIntentionsTaxiPathSnapper.Snap(
            MainSpurMain(3), JunctionEdges, new[] { "X" });
    ...
    [Fact]
    public void AnEmptyClearedSetProtectsNothing()
    {
        var result = SayIntentionsTaxiPathSnapper.Snap(
            MainSpurMain(3), JunctionEdges, System.Array.Empty<string>());
    ...
    [Fact]
    public void ClearedNamesAreMatchedCaseInsensitively()
    {
        var result = SayIntentionsTaxiPathSnapper.Snap(
            CyvrShape(), JunctionEdges, new[] { "y" });
