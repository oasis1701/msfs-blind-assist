...
//
// A COLD first Alt+L or Ctrl+Shift+L can wait 3-10 s with nothing said at all: up to 3 s for the
// OSM mirror, plus a first-time scenery scan and a census of the Community folder. A blind pilot
...
public class SurroundingsLookupNoticeTests
{
    ...
    public void The_delay_leaves_room_for_the_notice_itself_before_the_OSM_wait_expires()
        // "Looking around." is about 0.8 s of speech and the OSM bound alone is 3 s.
        => Assert.InRange(SurroundingsLookupNotice.Delay, TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(2.5));
}
