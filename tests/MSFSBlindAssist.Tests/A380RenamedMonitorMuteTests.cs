// FBW #10855 moved four A380 controls onto new variables (fixed 2026-09-25): the two flight-director
// combos (FD_1_CTL / FD_2_CTL, both Ctrl+M rows) became the ONE FD light, A32NX_FCU_FD_LIGHT_ON, the
// altitude increment moved from XMLVAR_AUTOPILOT_ALTITUDE_INCREMENT to the FCU's own
// A32NX_FCU_ALT_INCREMENT_1000, and the baro-unit selectors moved from XMLVAR_Baro_Selector_HPA_{1,2}
// to A32NX_FCU_EFIS_{L,R}_BARO_IS_INHG. A pilot who had muted the old row would suddenly hear the new one.
...
public class A380RenamedMonitorMuteTests
{
    ...
    // The per-side baro-unit rows: the EFIS-CP selector's own var since #10855 (found by review).
    [InlineData("XMLVAR_Baro_Selector_HPA_1", "A32NX_FCU_EFIS_L_BARO_IS_INHG")]
    [InlineData("XMLVAR_Baro_Selector_HPA_2", "A32NX_FCU_EFIS_R_BARO_IS_INHG")]
