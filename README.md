...
- A synthetic, audio Waypoint Flight Director for the en-route phase (Ctrl+F). Track your fixes in the 5 quick slots, optionally with crossing altitudes, and hand-fly to them by ear: a stereo-panned tone is the bank command and its pitch is the climb/descend command — you match it against a second tone of your actual attitude, the same idea as the landing guidance. Works on any aircraft with no autopilot or real flight director required; it fills the gap so you can hand-fly a climb, a vector or a leg to a fix instead of engaging the autopilot right after takeoff.
