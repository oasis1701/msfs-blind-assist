...
public class FBWA320SpeedWindow : FBWA320FCUWindowBase
{
    ...
    // Fenix-style: open SILENT. RequestFCUSpeedReadout forced a read that announced the
    // stale cached value first, then the fresh one (the "previous state, then the right
    // value" double). Value confirmed on Set; managed/selected auto-announces on change.
