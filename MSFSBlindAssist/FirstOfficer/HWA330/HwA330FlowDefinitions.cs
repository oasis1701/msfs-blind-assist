...
using Flow = Models.FlowDefinition<HwA330StateEvaluator>;
...
///   and isn't in HwA330StateEvaluator's PollFields) — mirrors the A380 flow's fixed-dwell
///   approach rather than the Fenix/PMDG N2-verification pattern.
/// </summary>
public static class HwA330FlowDefinitions
{
    ...
    // -----------------------------------------------------------------------
    // Step builders (mirror the Fenix/A380 files' helpers, retyped to HwA330StateEvaluator)
    // -----------------------------------------------------------------------
    ...
    private static Step Skip(Step step, Func<HwA330StateEvaluator, bool> cond)
    {
