...
using Item = Models.ChecklistItem<HwA330ActionExecutor, HwA330StateEvaluator>;
using Group = Models.ChecklistGroup<HwA330ActionExecutor, HwA330StateEvaluator>;
using CheckFn = System.Func<HwA330ActionExecutor, HwA330StateEvaluator, System.Threading.Tasks.Task>;

/// <summary>
/// Data-driven FlyByWire A320 (A32NX) First-Officer checklist definitions — the
/// STATE/ACTION layer that mirrors <see cref="HwA330FlowDefinitions"/> 1:1 (every
...
/// <see cref="HwA330StateEvaluator"/> — OnRequest vars are polled by the FO window
...
/// brightness: SET" per phase that fires the full <see cref="HwA330ActionExecutor.SetCockpitLighting"/>
/// scene; ECAM page items are ActionManual (a direct SD page-index write, held-safe —
/// see HwA330ActionExecutor.FireEcamPageAsync — but still fired as a one-shot checklist
/// action here rather than an auto-detected state, since auto-SD logic can override the
/// manual index on the next event).
/// </summary>
public static class HwA330ChecklistDefinitions
{
    ...
    private static Group BuildElectricalPowerUp() => new()
    {
        ...
        Items = new()
        {
            ...
            ActionManual("EPU_LTSCENE", "ELEC_POWER_UP", "Panel and integral brightness: SET",
                (e, _) => e.SetCockpitLighting(HwA330ActionExecutor.CockpitLightScene.DayPrep)),
        }
    };
    ...
    private static Group BuildAfterStart() => new()
    {
        ...
        Items = new()
        {
            ...
            ActionManual("AS_LTSCENE", "AFTER_START", "Panel and integral brightness: SET",
                (e, _) => e.SetCockpitLighting(HwA330ActionExecutor.CockpitLightScene.DimFlight)),
    ...
    private static Group BuildShutdown() => new()
    {
        ...
        Items = new()
        {
            ...
            ActionManual("SD_LTSCENE", "SHUTDOWN", "Panel and integral brightness: SET",
                (e, _) => e.SetCockpitLighting(HwA330ActionExecutor.CockpitLightScene.ParkingBright)),
    ...
    private static Group BuildSecure() => new()
    {
        ...
        Items = new()
        {
            ...
            ActionManual("SC_LTSCENE", "SECURE", "Panel and integral brightness: SET",
                (e, _) => e.SetCockpitLighting(HwA330ActionExecutor.CockpitLightScene.Off)),
        }
    };
    ...
    // -----------------------------------------------------------------------
    // Item builders (mirror the Fenix/A380 files' helpers, retyped to HwA330ActionExecutor/HwA330StateEvaluator)
    // -----------------------------------------------------------------------
