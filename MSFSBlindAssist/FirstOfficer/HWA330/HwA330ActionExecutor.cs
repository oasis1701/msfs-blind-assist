...
public sealed class HwA330ActionExecutor : IFoActionExecutor
{
    ...
    private bool ApplySilent(string varKey, double value)
    {
        ...
        try
        {
            ...
            {
                Log.Warn("fo_a330", $"refused write: '{varKey}' is registered as an EVENT and "
                    + $"HandleUIVariableSet does not claim it, so there is no way to fire it "
                    + $"(value {value}); nothing was written. Flow/checklist mapping bug — the "
                    + "key needs a definition branch or an executor dispatch arm.");
                return false;
            }
