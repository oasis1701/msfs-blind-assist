...
/// <summary>
/// The SimBridge half of the FlyByWire MCDU transport: connects to SimBridge's MCDU relay
/// websocket (<c>ws://host/interfaces/v1/mcdu</c>, the socket FBW's own remote MCDU web
/// UI uses) and exposes the Captain screen as <see cref="MCDUDisplayData"/>. Since the
/// Coherent debugger became the primary transport this is the FALLBACK for the screen
/// and the keys, and the ONLY source of MCDU printouts (ATIS/OFP <c>print:</c> messages
/// exist nowhere but on this relay).
///
/// Protocol: sim → client <c>update:{left:{…},right:{…}}</c>; client → sim
/// <c>event:left:&lt;KEY&gt;</c> and <c>requestUpdate</c>. The gateway echoes every
...
public sealed class FlyByWireSimBridgeMcduClient : IDisposable
{
    ...
    // outstanding send; SendButtonPress is fire-and-forget while the scratchpad typing
    // loop awaits a sequential per-character sequence, so two concurrent sends threw
    // InvalidOperationException (swallowed by SendRaw's catch → silently dropped keypress).
    ...
    public FlyByWireSimBridgeMcduClient(string host = "localhost:8380")
    {
        _host = host;
    ...
    private async Task ConnectLoop(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Log.Debug("Services", $"SimBridge MCDU connection error: {ex.Message}");
                    SetConnected(false);
                }
    ...
    private void HandleMessage(string msg)
    {
        ...
        // print: messages (ATIS/OFP print output) come before the update filter. FBW sends
        // print:{lines:[...]} where lines are plain strings (markup already stripped) that
        ...
        if (type == "print")
        {
            ...
            {
                Log.Debug("Services", $"SimBridge MCDU print parse error: {ex.Message}");
            }
            return;
        }
        ...
        {
            Log.Debug("Services", $"SimBridge MCDU parse error: {ex.Message}");
        }
    }
