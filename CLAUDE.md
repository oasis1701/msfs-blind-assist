...
- SayIntentions pushback approvals name the TAIL as a compass point ("Push and start approved. Tail South-West."), which tells a blind pilot nothing on its own — `Ctrl+S` converts it to the heading the NOSE finishes on plus the turn to get there. Nose = tail + 180; take the turn the SHORT way round (303→045 is 102 right, not 258 left); match compass points LONGEST NAME FIRST or "South-South-West" reads as "south" and the pilot is 22° out with nothing to reveal it. Parse only "tail" — no speculative "nose" branch — and require the approval wording so a stray mention of a tail isn't read as clearance to push. Position comes from the CACHE, never a fresh request: this hotkey is on the offline allowlist. **Unverified: whether SI's compass points are true or magnetic** — we assume true and convert the spoken heading to magnetic; a live pushback finishing ~one variation off is the tell. → [sayintentions.md](docs/sayintentions.md)
