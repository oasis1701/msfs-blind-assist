...
- SayIntentions pushback approvals name the TAIL as a compass point ("Push and start approved. Tail South-West."), which tells a blind pilot nothing on its own — `Ctrl+S` reads the controller verbatim and appends ONE word: `(right)` / `(left)` / `(about turn)`. Keep it one word. The earlier version spoke the finishing compass point, heading and turn size — accurate, and too much between hearing a clearance and acting on it. Parenthesised because screen readers don't speak brackets, so it arrives as a bare "right". Nose = tail + 180; take the turn the SHORT way round (303→045 is 102 right, not 258 left); match compass points LONGEST NAME FIRST or "South-South-West" reads as "south". Parse only "tail" — no speculative "nose" branch — and require the approval wording so a stray mention of a tail isn't read as clearance to push. Claim nothing under 10° or within 25° of a half-turn. Position comes from the CACHE, never a fresh request: this hotkey is on the offline allowlist. **Unverified: whether SI's compass points are true or magnetic** — we assume true; one word is what makes that safe to leave open, since a variation's worth of error can't flip left/right outside the guard bands. → [sayintentions.md](docs/sayintentions.md)
