...
- SayIntentions pushback approvals name the TAIL as a compass point ("Push and start approved. Tail South-West."), which tells a blind pilot nothing on its own — `Ctrl+S` reads the controller verbatim and appends the turn: `(straight)` / `(left)` / `(right)`. THREE answers, no magnitude, no special cases — earlier versions gave the finishing heading, the size of the turn and both ends of the aircraft ("tail left, nose right"); none of it was wanted and all of it sat between hearing a clearance and acting on it. Left/right is the change to the AIRCRAFT'S HEADING (303 + tail SW finishes on 045, so right). `straight` is an ANSWER, not silence; a pilot who hears nothing can't tell it from a readout that failed. Parenthesised because screen readers don't speak brackets. Measure the tail's departure from dead astern the SHORT way round or the wrap reads a 102° right as a 258° left; match compass points LONGEST NAME FIRST or "South-South-West" reads as "south". Parse only "tail" — no speculative "nose" branch — and require the approval wording so a stray mention of a tail isn't read as clearance to push. → [sayintentions.md](docs/sayintentions.md)
- Interpret SI's compass points against SI'S OWN `flight_details.heading`, never SimConnect's. Both come from SI, so the difference is exact and the unresolved question of whether SI means true or magnetic stops mattering. SimConnect is the fallback only, and carries a magnetic-variation error (14° at KBOS, more elsewhere) that is over half the 25° straight band. That band is sized by SI's EIGHT-point compass resolution (±22.5°), not by taste — do not tighten it; at that resolution a small turn and a straight push are the same message. Heading comes from a CACHE, never a fresh request: the hotkey is on the offline allowlist. → [sayintentions.md](docs/sayintentions.md)
...
- BOTH kinds of lost taxiway must be NAMED in the announcement — one the dialog could not seat AND one this airport does not have. `ParseTaxiways` returns only names the graph knows, so the second kind can only come from `ScanTaxiways`'s `Unresolved` half; reporting just the first leaves a pilot hearing a shorter route than ATC cleared with no way to see it. Keep `ParseTaxiways`'s signature working — it has callers and tests. → [sayintentions.md](docs/sayintentions.md)
- `current_flight.taxi_path` is GEOMETRY (`{"heading":…, "point":{"lon":…,"lat":…}}`), never taxiway names — do NOT re-add a reader for it, and do NOT re-add a structured branch that prefers it over the spoken clearance. The deleted reader accepted an object's `name`/`label`/**`id`**, which is exactly what a geometry array would gain: ~200 point ids would have become "taxiway names", armed the branch on their own, silently replaced the clearance-derived route and read the ids aloud as "could not apply". If SI ever publishes real names, rebuild it from a capture that shows them. → [sayintentions.md](docs/sayintentions.md)
