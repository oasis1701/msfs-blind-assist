...
- **Landing-exit safety net: runway-end countdown on `LoadRoute` failure.** `LandingExitPlanner.ActivateGuidance` no longer silently fails when route construction returns an error. It calls `TaxiGuidanceManager.BeginLandingRolloutNoRoute(runway)`, which routes through the existing `EnterRunwayEndCountdown()` to engage the per-frame distance callouts (1500 / 500 / 100 ft to runway end, speed-aware "slow down" / "stop" suffixes). The pilot loses tone guidance to the chosen exit but keeps the safety-critical countdown — much safer for a blind pilot on an active runway than the previous silent-failure path. `_activatedThisLanding` is set to `true` so the fallback is final for this landing; the planner doesn't retry on subsequent oleo bounces.
