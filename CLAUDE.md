...
- BOTH kinds of lost taxiway must be NAMED in the announcement — one the dialog could not seat AND one this airport does not have. `ParseTaxiways` returns only names the graph knows, so the second kind can only come from `ScanTaxiways`'s `Unresolved` half (speech) or `MatchKnownTaxiways`'s (structured `taxi_path`); reporting just the first leaves a pilot hearing a shorter route than ATC cleared with no way to see it. Keep `ParseTaxiways`'s signature working — it has callers and tests. → [sayintentions.md](docs/sayintentions.md)
