...
- **`Navigation/WaypointFlightDirectorGeometry.cs`** — pure static command math (probe-tested by `tools/WaypointFdProbe`, like `DockingGeometry`): `TrackError` (bearing-to-fix vs **GPS ground track**, so nulling it flies a wind-corrected straight line), `CommandedBankDeg` (proportional roll law `KRoll·(trackErr − yawRate·lead)` clamped to the bank cap — rate-lead off the track derivative kills overshoot), `RequiredFpaDeg` + `CommandedPitchDeg` (`pitch ≈ FPA + live AoA`, the VG nominal-pitch trick — no performance model), `ResolveVerticalTarget` (AT / AT_OR_ABOVE / AT_OR_BELOW / BETWEEN → command or neutral), `ProjectedCrossingAltFt`, `HasArrived` (capture radius OR abeam > 90° off track). `AltitudeConstraintType` lives here. **No TOD/TOC cue** — explicitly dropped per user (the tone is the instrument; when to descend is the pilot's call, esp. VFR).
- **`Services/WaypointFlightDirectorManager.cs`** — stateful shell: owns two `AudioToneGenerator`s (deferred Start, follower only if desired started — copied from VG), the leg sequencer (starts at the **first FILLED slot**, advances on arrival, **skips empty interior slots**, ends after the last filled slot or slot 5), per-frame command + render, the **ground-speed gate** (below the floor, ground track is unreliable → fall back to heading), **AP auto-mute** (tones silent while AUTOPILOT MASTER engaged), and the optional **centered tone change** (desired tone swaps to a chosen waveform within ±1.5° commanded bank). NEVER touches the controls. **Abeam (station-passage) arrival only counts when moving** (gs ≥ low-speed floor) so engaging while parked/behind a fix can't cascade through every slot; capture-radius arrival works at any speed. **Course legs**: capture-radius always sequences; an INBOUND course leg (`legStartDistNm > 4× capture radius`) ALSO sequences on abeam (so a leg passed wide doesn't strand the pilot), while an OUTBOUND radial (starts at/behind the fix) stays capture-only (abeam would misfire). **Slot-gap skipping + first-filled-slot start (2026-06):** `AdvanceLeg` `while`-skips empty slots and `Initialize` advances `activeSlot` to the first filled slot, so tracking into slots 1/2/4 (or a slot-3 leg that couldn't be tracked) flies 1→2→4 instead of dying at the gap; the MainForm engage guard is `!HasAnyWaypoint()` (was `IsSlotEmpty(1)`). **Descent-arm gate (2026-06, user-approved):** a crossing-altitude CLIMB commands immediately, but a DESCENT is held (vertical stays nominal/level) until the required FPA reaches `profile.DescentArmFpaDeg` (~3°) OR within `profile.VerticalArmRangeNm` (~25 NM) — so a far constrained STAR fix doesn't nudge a premature descent at cruise (tone-only; still no spoken TOD cue, honouring the user's "pilot decides when to descend"). `StandardBank` negates SimConnect's left-positive bank, same as VG.
- **Rides the existing `VISUAL_GUIDANCE_DATA` (req 505) stream** — MainForm feeds the FD the same `VISUAL_GUIDANCE_*` events as VG via sibling handler blocks (FD and VG are **mutually exclusive**; activating one stops the other). The stream is now **reference-counted** (`SimConnectManager.Acquire/ReleaseVisualGuidanceMonitoring`) so neither feature stops it out from under the other. **⚠️ Each feature must Release only what it Acquired** — MainForm gates Acquire/Release with per-feature `_vgHoldsStream`/`_fdHoldsStream` flags, because an aborted activation (FD with empty slot 1, VG with no runway) runs its `Stop`→deactivate path and would otherwise release a claim it never took, killing the OTHER feature's stream (caught in the 5-pass review). **Both are also stopped in `SwitchAircraft`** (they were Initialized with the old aircraft's profile/runway). `AUTOPILOT MASTER` was added as the last `VisualGuidanceData` field + a `VISUAL_GUIDANCE_AP_MASTER` event (VG ignores it). **⚠️ AP-mute detection: the FBW Airbuses (A320/A380/A330) do NOT drive the stock `AUTOPILOT MASTER` simvar — they use `A32NX_AUTOPILOT_1/2_ACTIVE`** — so the `VISUAL_GUIDANCE_AP_MASTER` MainForm handler OR's the stock value with those cached FBW vars (`GetCachedVariableValue`). Without that the FD auto-mute silently never fired on FBW aircraft (reported live). PMDG/Fenix/HS787 set the stock var.
...
- **EFB (Shift+E) "Track Slot N" opens the Track Fix dialog PRE-FILLED (not a silent track).** History: it first called the bare `TrackWaypoint(slot, fix)` (dropped altitude/course); then mapped + tracked *silently*, but the constraint was then invisible/uneditable (the Shift+F dialog didn't reflect it — user report). Now `ElectronicFlightBagForm.TrackSlotMenuItem_Click` (after the null + (0,0) guards) raises **`event Action<WaypointFix,int> TrackToSlotRequested`**; MainForm (`OnEfbTrackToSlotRequested`, subscribed at EFB creation) lazily builds the dialog (`EnsureTrackFixForm`, shared with the Shift+F path) and calls **`TrackFixForm.ShowFormPrefilled(fix, slot)`** — fills ident/slot + runs `WaypointConstraintMapper.FromFix` to pre-fill crossing-alt/constraint/upper-alt/course for review+edit, commits on Track. The dialog carries the pre-resolved fix in **`_prefilledFix`** so Track uses its exact coords (incl. navaid/runway fixes not in the `waypoint` table) instead of re-searching the ident — UNLESS the pilot edits the name, then it falls back to a fresh `GetWaypointsByIdent` search. `_prefilledFix` is cleared in `SwitchToSearchMode` so a plain Shift+F open never reuses it. **The Upper Altitude label+box are shown only for the "Between" constraint** (`UpdateUpperAltVisibility`, wired to `constraintComboBox.SelectedIndexChanged`; hidden → Tab/screen-reader skips them + can't be filled by mistake). The mapper (pure, probe-tested) → `(crossingAlt, upperAlt, AltitudeConstraintType, course)` switches on the **raw ARINC `alt_descriptor`** — now carried on `WaypointFix.AltDescriptor`, set in `ParseLegToWaypoint` (the unambiguous type source; the formatted `AltitudeRestriction` string is only a fallback for non-navdata fixes, since a single-bounded `B` or any descriptor `FormatAltitudeRestriction` doesn't special-case would otherwise mis-type to a hard "At"). NUMBERS come from the robust `MinAltitude`(=alt1)/`MaxAltitude`(=alt2) ints; a two-bound `B` → Between (lower/upper ordered), a single-bound/equal `B` → AtOrAbove (the floor, never dropped); course only when `>0` and `!IsTrueCourse` (true-course legs would need the fix's mag-var, so they fall back to direct-to). The four descriptors cover ~100% of altitude-bearing legs in the navdata (live count: `+` 223k, `A` 59k, `−` 14k, `B` 10k; 0 true-course, 0 zero-bound `B`). **⚠️ The handler REJECTS a position-less fix** (`Latitude==0 && Longitude==0`) — ARINC maneuver legs (CA/VA/VM/FM/CI/VI/CD/VD/CR, ~14% of legs) and unresolved fixes parse to (0,0); tracking one would steer the FD toward null island, so it announces "&lt;ident&gt; has no position and cannot be tracked" instead.
...
**Per-aircraft tuning = `WaypointFlightDirectorProfile` via `IAircraftDefinition.GetWaypointFlightDirectorProfile()`** (KRoll, MaxBank/MaxPitch, capture radius, low-speed floor, rate-lead, AoA fallback, tone Hz/range). A320 baseline (FBW A32NX = NEO, Fenix = CEO); gentler roll gain + larger capture + longer lead for PMDG 777, HS 787 and FBW A380X (they roll slower / cover ground faster). **⚠️ These are best-effort class defaults — the gains/caps/capture-radius need LIVE in-sim tuning** (no AP to verify against; this is hand-flying). Tune like the taxi-turn-lead and VG profiles were: fly each aircraft, adjust the profile values. Settings (`UserSettings.WaypointFd*`, all in the Hand Fly Options dialog): tone waveform/volume (desired + current), hard-pan, AP-auto-mute (default on), and the **centered tone change** (toggle + waveform, default off).

**Centered tone change (FD + VG, 2026-06).** Optional, off by default, in Hand Fly Options. When on, the *desired* (command) tone switches to a user-chosen waveform while laterally centered (|commanded bank| ≤ 1.5°) and back to its normal waveform off-track — an extra timbre cue for centered vs not, on top of the pan. Only the desired tone changes (stays distinct from the current tone; no phase-cancel). Implemented identically in `WaypointFlightDirectorManager.ApplyCenteredWaveform` and `VisualGuidanceManager` (settings `{VisualGuidance,WaypointFd}CenteredToneEnabled/Waveform`). Short checkbox labels, full detail in the accessible hints; the new options ride `SettingsManager.Current` (committed on OK) rather than the HandFlyOptionsForm constructor.

**A380X + HS787 Visual Guidance profiles corrected (2026-06).** Both previously inherited the A320 *baseline* VG profile (flares late / reads high for a heavy widebody); now carry 777-class `GetVisualGuidanceProfile()` overrides (modelled on the measured 777 + type performance — the installed `flight_model.cfg` carry only vestigial stub speeds, A380 == A320, so NOT file-derived; estimates pending in-sim calibration; A380 `FlareTriggerWheelHeightFt`=50 per the user-confirmed RA callout). See the Visual Guidance section.

**Course / radial tracking (FD option 3, 2026-06).** An optional per-slot magnetic **Course** (Track Fix window). With it set, that leg captures and HOLDS the course line through the fix (airway/approach course/radial) instead of direct-to — the generalised ILS localizer capture: `WaypointFlightDirectorGeometry.CrossTrackNm` (signed great-circle cross-track, + = right of course) + `CourseInterceptTrackDeg` (intercept angle ramped to `MaxInterceptDeg`/`InterceptDegPerNm`), wind-corrected via ground track. **Course legs sequence on capture-radius ONLY** (no abeam — an outbound radial starts behind the fix where abeam would misfire). `WaypointSlotData.Course`; profile gains `MaxInterceptDeg`/`InterceptDegPerNm`.

**"Fly a heading" = course tracking, NOT a separate bug mode.** A standalone heading/altitude bug mode was briefly added then REMOVED (2026-06, user decision) — flying a specific heading the FMC/MCDU gives is covered by **course tracking** (set a course on the slot → capture & hold that course through the fix). Don't re-add a bugless heading mode.

**Command slewing (anti-fluctuation, 2026-06).** `SlewCommands` rate-limits the rendered bank + pitch commands between frames (`profile.MaxBankRateDegPerSec`/`MaxPitchRateDegPerSec`, dt from `DateTime.UtcNow`, re-seeded each session via `cmdInit`), applied in ALL modes (slot / course / bug) just before rendering — fixes the steering tone wobbling on track/heading jitter (the reported "it fluctuates"). VG already rate-limits; the FD now does too.

**Rudder-coordination slip cue (option 4, 2026-06) — independent of the FD.** Output-mode **Ctrl+K** toggles `Services/SlipCueGenerator.cs`: a HARD-PANNED white-noise tick ("step on the ball") entirely in the ear of the rudder to press (ball left→left ear→left rudder), tick rate rising with how far the ball is out, silent when coordinated. Self-contained NAudio (a stereo white-noise `ISampleProvider` with a sample-accurate on/off rhythm; no AudioToneGenerator since that's tone-only). Driven by **`TURN COORDINATOR BALL`** added as an always-on SIM_FRAME base var (`BaseAircraftDefinition.GetBaseVariables`, like G_FORCE), routed in `MainForm.HandleSpecialAnnouncements` only when toggled on (normalised to ~±1; positive = ball right). Runtime toggle, default off; volume = `UserSettings.SlipCueVolume`. **Ball sign + the `Position` unit are to be confirmed in-sim** (one-line flip if reversed). Works during FD, VG, or pure hand-fly. **Stopped in `SwitchAircraft` (and `_slipCueOn` reset) + disposed in `OnFormClosing`** — it owns its own `WaveOut`, independent of the FD/VG managers, so it must be torn down explicitly or the tick carries onto the next aircraft / leaks on close.

**FD bug-check fixes (2026-06, post options 3/4).** Three real issues from the full FD review, fixed: (1) the slip cue's `WaveOut` was never stopped on aircraft swap / close (above); (2) `TrackFixForm.SwitchToSearchMode` set `ClientSize` height 375 while the search-mode content (the new Course field + Track button) reached y≈415 — clipped on every open; now 425 (Designer is 435); (3) `WaypointFlightDirectorManager.UpdateYawRate` derived the rate-lead from raw ground track even below the speed floor where the lateral error had switched to heading — now it takes `effectiveTrack` (the same signal the error uses) and re-baselines while AP-muted, so no noisy/stale lead in slow flight or after AP disengage.
...
- `GetRunwayDetailedInfo` must explicitly alias the `runway_end` columns (`re.heading AS end_heading`, etc.) — the bare columns are ambiguous between `runway`/`runway_end` and silently resolve to the primary-end value. → [architecture.md](docs/architecture.md)
...
- Never scan `%APPDATA%\HiFi\` for the ActiveSky settings-file port — recursive directory enumeration there blocks the UI thread for many seconds. → [taxi-guidance.md](docs/taxi-guidance.md)
- A "METAR says no precipitation" result must render as "None," never fall through to the next weather source — only a wholly-missing METAR should trigger fallback. → [taxi-guidance.md](docs/taxi-guidance.md)
- Turbulence ≤25 (the AS calm-weather baseline) must be hidden entirely, never shown as a raw alarming number. → [taxi-guidance.md](docs/taxi-guidance.md)
- Don't move the WMO/ICAO weather-token decoder out of `WeatherRadarForm.ParsePrecipFromMetar` without updating its duplicate in the weather monitor — keep both copies in sync. → [taxi-guidance.md](docs/taxi-guidance.md)
