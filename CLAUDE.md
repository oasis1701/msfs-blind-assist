...
- SayIntentions pushback approvals name the TAIL as a compass point ("Push and start approved. Tail South-West."), which tells a blind pilot nothing on its own — `Ctrl+S` reads the controller verbatim and appends the pushback to ASK FOR: `(straight)` / `(tail left, nose right)` / `(tail right, nose left)` / `(about turn)`. Name BOTH ends — the phrasing has to map onto the GSX menu, which leads with either. `straight` is an ANSWER, not silence; a pilot who hears nothing can't tell it from a readout that failed. Parenthesised because screen readers don't speak brackets. Measure the tail's departure from dead astern the SHORT way round (heading 303, tail SW = 102° clockwise, not 258° the other way); match compass points LONGEST NAME FIRST or "South-South-West" reads as "south". Parse only "tail" — no speculative "nose" branch — and require the approval wording so a stray mention of a tail isn't read as clearance to push. Claim nothing within 25° of a half-turn. → [sayintentions.md](docs/sayintentions.md)
