...
- The ActiveSky precip auto-announce must NOT repeat an unchanged phrase: compare the decoded precip phrase trimmed + case-insensitive, and speak only on start / stop / a genuinely different phrase ("light rain" → "light rain" stays silent). → [taxi-guidance.md](docs/taxi-guidance.md)
- Under ActiveSky the SimConnect ambient feed is unreliable: source the Alt+W precip auto-announce AND the output+I wind from ActiveSky (position/station METAR for precip, AS ambient wind + surface gust for wind) — the SimConnect `AMBIENT PRECIP STATE` bitmask sticks and the SimConnect ambient wind lags AS, so output+I disagreed with the radar's "wind at altitude." → [taxi-guidance.md](docs/taxi-guidance.md)
...
- The 1,000-ft altitude callout (`AltitudeCalloutAnnouncer`) must fire AT the boundary (the band change IS the crossing), not ~80 ft late — the hysteresis is a same-boundary hover DEBOUNCE only (suppresses re-crossings of the just-announced thousand), never a delay on the first crossing; and it announces the thousand CROSSED (same value climbing or descending), not the band entered. → [taxi-guidance.md](docs/taxi-guidance.md)
...
### First Officer Automation (PMDG 777/737, Fenix A320, FBW A380) (→ [first-officer.md](docs/first-officer.md))
...
- PMDG auto-flaps was REMOVED (2026-07-08, user decision) — never reintroduce a 737/777 flap schedule; the "Auto-manage flaps" setting acts on the FBW A380 only (Fenix stores-but-ignores it, the PMDG managers now do the same). → [first-officer.md](docs/first-officer.md)
